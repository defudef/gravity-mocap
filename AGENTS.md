...
- The approved `core` corpus is CMU + AddBiomechanics `train/With_Arm` + 100STYLE. SAM, mRI, and HUM4D remain catalogued but blocked from official checkpoint training until their raw-data rights/provenance concerns are resolved. TUM is motion supervision, not paired video, and requires explicit DUA acceptance. Ignore bundled mRI pretrained `.pkl` models. HUM4D SMPL/SMPL-X parameters are prohibited.
...
- `./scripts/start-fresh-training.sh --execute --max-hours HOURS` and `./scripts/start-fresh-training.sh --execute --max-epochs N` are the path-independent one-command bootstraps for a new run. They may archive the current default output only after CMU/AddBiomechanics/100STYLE download, preprocessing, shard checks, and a fresh training dry-run succeed. Never use them to resume; use `scripts/train.sh` for subsequent sessions.
- `./scripts/train-small.sh` selects the 6x384 capacity baseline and its isolated `runs/motion-small` output. Keep its seed, data split, augmentation, optimizer, validation, and early stopping aligned with `train-paper.yaml` so comparisons change model capacity only.
...
- Preserve canonical 30 FPS preprocessing, per-shard source/target FPS provenance, B3D trial identity, source-file/style-group holdout, configurable synthetic-camera provenance, and epoch/sequence/window-derived detector augmentation. A resumed batch must receive the same corrupted detector inputs.
...
- The default clean-room video frontend is the checksum-pinned MediaPipe Pose Landmarker Heavy bundle through `./scripts/video.sh`. Preserve its 33-to-22 mapping, heel/toe landmarks, VIDEO tracking mode, content-addressed/idempotent cache, full source/model/checkpoint provenance, and fail-closed handling of long detection gaps. Do not replace it with weights whose training-data terms are unknown or non-commercial without a new license audit.
- Video inference artifacts stay below `Saved/GravityMocap/inference/` and are not training shards. Preserve finite-value validation, edge padding for short clips, overlap blending, rotation re-normalization, metres-per-second root integration divided by FPS, and visual previews for both 2D detection and neutral 3D output.
